...
if [ $1 ]; then
	...
	*)
	...
	"x86_64" | *"amd64"*)
		...
		#docker restart emby
		docker cp /root/plugins/Emby.MeiamSub.Shooter.dll emby:/system/plugins/
		docker cp /root/plugins/Emby.MeiamSub.Thunder.dll emby:/system/plugins/
		docker cp /root/plugins/Emby.MeiamSub.Shooter.dll emby:/config/plugins/
		docker cp /root/plugins/Emby.MeiamSub.Thunder.dll emby:/config/plugins/
		docker exec -i emby chmod 755 /config/plugins/Emby.MeiamSub.Shooter.dll
		docker exec -i emby chmod 755 /config/plugins/Emby.MeiamSub.Thunder.dll
		docker restart emby
		echo "一键全家桶全部安装完成"
		;;
	"aarch64" | *"arm64"* | *"armv8"* | *"arm/v8"*)
		docker run -d --name emby -v $1/config:/config -v $1/xiaoya:/media --network=container:xiaoya --user 0:0 --restart always emby/embyserver_arm64v8:4.8.0.56
		#docker cp /tmp/Emby.Server.Implementations.dll emby:/system/
		#docker exec -i emby chmod 644 /system/Emby.Server.Implementations.dll
		#docker restart emby
		echo "一键全家桶全部安装完成"
